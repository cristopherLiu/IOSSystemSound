...
class DataService {
  ...
  func fetchData(onComplete: ([Sound]) -> Void) {
    
    guard let category = library["category"]  else {
      return
    }
    let sounds = category.flatMap({ c in c.value.map({ Sound(category: c.key, fileName: $0.key, id: $0.value) })  })
    onComplete(sounds)
  }
